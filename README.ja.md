...
| マルチエンジン | ✅ OpenCode / Copilot / Claude Code を切り替え | ❌ 単一モデル / プロバイダー |
...
スクリーンショットを貼り付け、デザインモックアップをドラッグ、エラー画像をアップロード — 3つのエンジンすべてが画像をネイティブに分析できます。各エンジンアダプターが裏で画像をネイティブフォーマットに変換し、統一された体験を提供します：
...
| 方法 | 設定 | 最適な用途 |
|------|------|-----------|
| **LANブラウザ** | `http://<あなたのIP>:5173` を開き、6桁コードを入力またはQRスキャン | 同じネットワーク上の別デバイスからの高速アクセス |
